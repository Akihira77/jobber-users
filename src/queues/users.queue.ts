...
export class UsersQueue {
	...
	async publishDirectMessage(
		...
		try {
			...
			this.logger("queues/users.producer.ts - publishDirectMessage()").info(
				logMessage,
			);
		} catch (error) {
			this.logger("queues/users.producer.ts - publishDirectMessage()").error(
	...
	async consumeBuyerDirectMessages(): Promise<void> {
		try {
			...
			await this.ch.bindQueue(jobberQueue.queue, exchangeName, routingKey);
			this.ch.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
				try {
			...
			this.ch.consume(jobberQueue.queue, async (msg: ConsumeMessage | null) => {
				try {
					...
					if (type === "create-order") {
						...
						this.ch!.ack(msg!);
						this.logger("queues/users.queue.ts").info(
							`received data to update gig count for sellerId ${sellerId}`,
						);
						return;
					} else if (type === "cancel-order") {
						await this.sellerService.updateSellerCancelJobsProp(sellerId);
						this.ch!.ack(msg!);
						return;
					}
	...
	async consumeReviewFanoutMessages(): Promise<void> {
		try {
			...
			const { exchangeName } = reviewServiceExchangeNamesAndRoutingKeys.review;
				...
				try {
					...
					if (type === "addReview") {
						...
						if (parsedData.type === "buyer-review") {
							await this.sellerService.updateSellerReview(parsedData);
			...
			this.ch.consume(
				...
				async (message: ConsumeMessage | null) => {
					...
					if (type === "getSellers") {
						...
						await this.sellerService.getRandomSellers(parseInt(count));
